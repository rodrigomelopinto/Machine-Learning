...
for a in ks:
    model = tree.DecisionTreeClassifier(max_depth=a)
    scores = cross_validate(model, df1,df.Class, cv=KFold(n_splits=10,random_state=27,shuffle=True), scoring='accuracy', return_train_score=True)
